...
const renderCard = (prescription) => {
    ...
    const formattedDate = new Date(prescription.createdAt).toLocaleDateString('en-GB', options);
    return(
        <CardContent>
            ...
            <Grid container spacing={2} sx={{ fontWeight: 'bold' }}>
                <Grid item xs={3}><Typography sx={{ color: 'text.primary', fontSize: { xs: 10, sm: 16 } }}>Medicine Name</Typography></Grid>
                <Grid item xs={2}><Typography sx={{ color: 'text.primary', fontSize: { xs: 10, sm: 16 } }}>Dosage</Typography></Grid>
                <Grid item xs={2}><Typography sx={{ color: 'text.primary', fontSize: { xs: 10, sm: 16 } }}>Frequency</Typography></Grid>
                <Grid item xs={2}><Typography sx={{ color: 'text.primary', fontSize: { xs: 10, sm: 16 } }}>Duration</Typography></Grid>
                <Grid item xs={3}><Typography sx={{ color: 'text.primary', fontSize: { xs: 10, sm: 16 } }}>Instructions</Typography></Grid>
            </Grid>

            {prescription.medicines.map((medicine, index) => (
                <Grid container spacing={2} key={index} sx={{ mt: 2 }}>
                    <Grid item xs={3}><Typography>{medicine.name}</Typography></Grid>
                    <Grid item xs={2}><Typography>{medicine.dosage}</Typography></Grid>
                    <Grid item xs={2}><Typography>{medicine.frequency}</Typography></Grid>
                    <Grid item xs={2}><Typography>{medicine.duration}</Typography></Grid>
                    <Grid item xs={3}><Typography>{medicine.instructions}</Typography></Grid>
                </Grid>
            ))}
        </CardContent>
    )
                            ...
                            {filteredPrescriptions?.length > 0 ? (
                                filteredPrescriptions.map((prescriptionData, index) => (
                                    <Grid item xs={12} md={6} key={index}>
                                        <Card sx={{minWidth:'40vw'}} variant="outlined">{renderCard(prescriptionData)}</Card>
                                    </Grid>
                                ))
                            ) : (
                                <Grid item xs={12}>
